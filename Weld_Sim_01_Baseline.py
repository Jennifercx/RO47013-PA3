...
# ==================== Path Definition (Baseline Trajectory) ===================== #

num_seam_segments = 50
px_vals = np.linspace(-0.4, 0.4, num_seam_segments + 1)
# Baseline path: different from training sine by mixing two wavelengths.
# This keeps it smooth but less predictable, useful for reactive capability testing.
py_vals = (
    0.018 * np.sin(px_vals * 2 * np.pi / 0.17 + 0.6)
    + 0.010 * np.sin(px_vals * 2 * np.pi / 0.33 - 1.1)
)
